...
让模型把一次回复拆成 1~3 条短消息，用 `|||` 分隔，App 切成多个气泡挂在同一个头像下。
因为分条是跟着流式输出走的，所以气泡会**一条条自然冒出来**，不是一起闪出来。

关掉就退回「一次回一条长的」。

> 代价：模型偶尔可能在正常句子里写出 `|||`，那样会多切一个气泡。概率很低，无害。

### 主动问候（默认关）

你长时间不说话，它主动找你。

**这一项默认关闭，因为它会在你不操作时持续调用 API（产生费用）。** 打开前请想清楚：
手机放着不动、App 停在聊天页，它每隔一段时间就会发一条。

做了四重刹车，任何一条不满足都不会触发：

1. **仅前台** —— App 切到后台立即停止
2. **仅聊天页** —— 停在角色列表或设置页不会触发
3. **连续上限** —— 默认连发 3 条后自动安静，直到你回一句话才重新计数
4. **空闲阈值** —— 默认静默满 10 分钟才开口

另外刚进聊天页有 30 秒冷却，不会一坐下角色就自己说话。
两个参数（静默分钟数 / 连续上限）都能在设置页调。

主动问候**只发文字，不会主动发图** —— 否则每次问候都可能触发一次生图调用。

### 表情包

需要先配好生图接口（见上一节）。

- **emoji**：模型本来就会用，没限制，零成本
- **真·表情包图片**：模型输出 `[[STICKER: 描述]]`，App 用固定画风
  （简笔画卡通、纯白背景、粗线条、夸张表情）生成一张小图，压到 320px 显示

表情包可以单独关掉（设置页 → 图片生成 → 允许角色发表情包），
关掉后模型仍能用 emoji，只是不会生成贴纸图。

---

## 附：让角色发语音（可选，接火山引擎）

默认**关闭**。开启后角色会输出 `[[VOICE: 要说的话]]`，由[火山引擎豆包语音](https://console.volcengine.com/speech/app)
合成成 mp3 发出来，点气泡播放。

> **只做「角色发语音」，不支持你录音。** 所以不需要麦克风权限。

### 怎么配

1. 去 [火山引擎语音技术控制台](https://console.volcengine.com/speech/app) 创建应用（有免费额度）
2. 拿到三样东西：**App ID**、**Access Token**、**Cluster**
3. 设置页 → 语音合成 → 勾上 → 填进去 → 点「测试语音」

| 字段 | 说明 |
|---|---|
| App ID | 控制台的应用 ID |
| Access Token | 控制台的 Access Token |
| Cluster | 音色所属集群。**标准音色一般是 `volcano_tts`**，大模型音色和复刻音色不一样，填错会报错 |
| 默认音色 | 如 `BV001_streaming`，控制台音色列表里有 |
| 语速 | 0.2 ~ 3.0 |

**每个角色可以配不同嗓子**：角色编辑弹窗里有「语音音色」，留空则用全局默认。

### 四个实现上的取舍

**1. 用的是 V1 接口，官方标注「不推荐」。** 火山推荐 V3，但 V3 只有 WebSocket / HTTP Chunked / SSE
三种**流式**接口，音频是二进制自定义分帧，解析复杂度高一个量级。V1 是一次请求返回全部音频，
短语音完全够用。如果哪天 V1 下线了，再上 V3 的 SSE。

**2. 鉴权头是 `Bearer;token`，分号不是空格。** 这是火山特有的写法，写成空格会直接 401。

**3. 必须显式指定 `encoding: "mp3"`。** 火山默认返回 **pcm**，浏览器 `<audio>` 播不了。
自检脚本里有专门的断言守着这一条。

**4. HTTP 200 不代表合成成功。** 要看响应体里的业务码 `code === 3000`，否则会把失败当成功。

### 静音开关

iOS 上 WKWebView 的媒体播放默认走 `AVAudioSessionCategoryAmbient`，**会被手机侧面的静音开关静音**。
语音消息显然要能听见，所以播放前 App 会通过 `audioSession` 原生通道把类别切到 `Playback`，
播完再切回来（不影响你其他 App 的音乐）。这是本次唯一的原生改动。

### 存储

语音和图片、表情包**共用同一份 2.5MB 预算**。一条 5 秒 mp3 大约 30~60KB（base64 后约 40~80KB），
所以语音多了会更快触发清理——清理时从最旧的开始，**文字永远不会被清**，气泡里会显示「语音已清理」。

导出的 JSON 里保留语音的**文字内容**，但不含音频数据。

### 为什么语音气泡下面有字幕

气泡是微信那种 `▶ ▂▃▅ 4″` 播放器，下面用暗色小字显示说的话。这不是多余：

- 手机静音或戴耳机时不用点开也知道说了什么
- 合成失败时文字仍然可见，不会整条消息消失
- 想复制内容时有地方可复制
